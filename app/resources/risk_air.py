...
class RiskAirResource(Resource):
    def get(self):
        args = _parser.parse_args()
        validate_coords(args["lat"], args["lon"])

        resp = _PROVIDER.air(args["lat"], args["lon"])

        aqi = resp["us_aqi"]
        if aqi <= 50:        level, label = 0, "Baixo"
        elif aqi <= 100:     level, label = 1, "Moderado"
        elif aqi <= 150:     level, label = 2, "Alto"
